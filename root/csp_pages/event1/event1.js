...
function displayEventData(eventData) {
    ...
    const dataDisplayDiv = document.getElementById("event1Data"); // Div to display event1 data
...
// Function to fetch and display data for event1
function fetchEvent1Data() {
    const event1Ref = ref(database, 'events/event1'); // Reference to 'event1' data in Firebase
    ...
    get(event1Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                displayEventData(eventData); // Display the fetched event1 data
            } else {
                document.getElementById("event1Data").innerHTML = '<p>No data available for event1.</p>';
            }
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
            document.getElementById("event1Data").innerHTML = `<p>Error fetching data: ${error.message}</p>`;
        });
}

// Fetch and display data when the page loads
window.onload = fetchEvent1Data;
