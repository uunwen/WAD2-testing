...
function displayEventData(eventData) {
    ...
    const dataDisplayDiv = document.getElementById("event12Data"); // Div to display event12 data
    ...
    displayOrder.forEach(key => {
        ...
        if (key === 'Organiser' && eventData[key]) {
            ...
            organiserLink.href = '../../sponsor_pages/sponsor12/sponsor12.html'; // Correct relative path
...
// Function to fetch and display data for event12
function fetchEvent12Data() {
    const event12Ref = ref(database, 'events/event12'); // Reference to 'event12' data in Firebase
    ...
    get(event12Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                displayEventData(eventData); // Display the fetched event12 data
            } else {
                document.getElementById("event12Data").innerHTML = '<p>No data available for event12.</p>';
            }
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
            document.getElementById("event12Data").innerHTML = `<p>Error fetching data: ${error.message}</p>`;
        });
}

// Fetch and display data when the page loads
window.onload = fetchEvent12Data;
