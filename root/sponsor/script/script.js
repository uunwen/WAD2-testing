...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchStudentsForEvent(eventKey) {
    ...
    database.ref("events/" + eventKey + "/signups").once(
      "value",
      function (snapshot) {
        ...
        if (capacity > 0) {
          console.log("Calling updatePieChart with:", {
            studentCount,
            capacity,
          });
    ...
    database.ref("events/" + eventKey).once(
      "value",
      function (snapshot) {
  ...
  function fetchSponsorOrgName(uid) {
    database.ref("sponsors/" + uid).once(
      "value",
      function (snapshot) {
...
function updatePieChart(studentCount, capacity) {
  ...
  const signupCapacityChart = document
    .getElementById("signup-capacity-chart")
    .getContext("2d");
