...
document.getElementById("submitEvent").addEventListener("click", async function (event) {
  ...
  const requiredFields = [
    ...
    { id: "capacity", message: "Capacity must be a number greater than or equal to 1.", isNumber: true },
    ...
    { id: "projectRequirements", message: "Project Requirements cannot be empty." },
    { id: "region", message: "Please select a region." },
    { id: "sessions", message: "Session details cannot be empty." },
    { id: "totalCSPHours", message: "Total CSP hours must be a number greater than or equal to 1.", isNumber: true },
    { id: "volunteerPeriodStart", message: "Volunteer Period Start cannot be empty." },
    { id: "volunteerPeriodEnd", message: "Volunteer Period End cannot be empty." },
  ...
  if (startDateInput.value && endDateInput.value) {
    if (endDate <= startDate) {
      ...
      if (!errorElement || !errorElement.classList.contains('error-message')) {
        errorElement = document.createElement('div');
        errorElement.className = 'error-message';
        errorElement.textContent = "Volunteer period end date must be later than the start date.";
  ...
  try {
    // Get input values
    const admissionsPeriod = document.getElementById("admissionsPeriod").value;
