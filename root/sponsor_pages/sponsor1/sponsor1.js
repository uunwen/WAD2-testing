...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor1 data
    ...
    aboutSection.innerHTML = `<strong>About:</strong> ${sponsorData['org_background'] || 'No information available.'}`;
...
// Function to fetch and display data for sponsor1
function fetchSponsor1Data() {
    const sponsor1Ref = ref(database, 'sponsors/sponsor1'); // Reference to 'sponsor1' data in Firebase

    get(sponsor1Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor1 data
            } else {
...
window.onload = fetchSponsor1Data;
