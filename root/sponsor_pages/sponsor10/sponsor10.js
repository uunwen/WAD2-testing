...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor10 data
...
// Function to fetch and display data for sponsor10
function fetchSponsor10Data() {
    const sponsor10Ref = ref(database, 'sponsors/sponsor10'); // Reference to 'sponsor10' data in Firebase

    get(sponsor10Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor10 data
            } else {
...
window.onload = fetchSponsor10Data;
