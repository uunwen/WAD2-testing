...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor2 data
...
// Function to fetch and display data for sponsor2
function fetchSponsor2Data() {
    const sponsor2Ref = ref(database, 'sponsors/sponsor2'); // Reference to 'sponsor2' data in Firebase

    get(sponsor2Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor2 data
            } else {
...
window.onload = fetchSponsor2Data;
