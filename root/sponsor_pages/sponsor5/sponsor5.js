...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor5 data
...
// Function to fetch and display data for sponsor5
function fetchSponsor5Data() {
    const sponsor5Ref = ref(database, 'sponsors/sponsor5'); // Reference to 'sponsor5' data in Firebase

    get(sponsor5Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor5 data
            } else {
...
window.onload = fetchSponsor5Data;
