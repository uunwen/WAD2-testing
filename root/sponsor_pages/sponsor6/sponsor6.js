...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor6 data
...
// Function to fetch and display data for sponsor6
function fetchSponsor6Data() {
    const sponsor6Ref = ref(database, 'sponsors/sponsor6'); // Reference to 'sponsor6' data in Firebase

    get(sponsor6Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor6 data
            } else {
...
window.onload = fetchSponsor6Data;
