...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor7 data
...
// Function to fetch and display data for sponsor7
function fetchSponsor7Data() {
    const sponsor7Ref = ref(database, 'sponsors/sponsor7'); // Reference to 'sponsor7' data in Firebase

    get(sponsor7Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor7 data
            } else {
...
window.onload = fetchSponsor7Data;
