...
function displaySponsorData(sponsorData) {
    ...
    const dataDisplayDiv = document.getElementById("sponsorData"); // Div to display sponsor9 data
...
// Function to fetch and display data for sponsor9
function fetchSponsor9Data() {
    const sponsor9Ref = ref(database, 'sponsors/sponsor9'); // Reference to 'sponsor9' data in Firebase

    get(sponsor9Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                const sponsorData = snapshot.val();
                displaySponsorData(sponsorData); // Display the fetched sponsor9 data
            } else {
...
window.onload = fetchSponsor9Data;
