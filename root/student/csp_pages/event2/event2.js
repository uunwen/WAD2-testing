...
function displayEventData(eventData) {
    ...
    const dataDisplayDiv = document.getElementById("event2Data"); // Div to display event2 data
    ...
    displayOrder.forEach(key => {
        ...
        if (key === 'Organiser' && eventData[key]) {
            ...
            organiserLink.href = '../../../student/sponsor_pages/sponsor2/sponsor2.html'; // Correct relative path
...
// Function to fetch and display data for event2
function fetchEvent2Data() {
    const event2Ref = ref(database, 'events/event2'); // Reference to 'event2' data in Firebase
    ...
    get(event2Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                displayEventData(eventData); // Display the fetched event2 data
            } else {
                document.getElementById("event2Data").innerHTML = '<p>No data available for event2.</p>';
            }
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
            document.getElementById("event2Data").innerHTML = `<p>Error fetching data: ${error.message}</p>`;
        });
}

// Fetch and display data when the page loads
window.onload = fetchEvent2Data;
