...
function displayEventData(eventData) {
    ...
    const dataDisplayDiv = document.getElementById("event4Data"); // Div to display event4 data
    ...
    displayOrder.forEach(key => {
        ...
        if (key === 'Organiser' && eventData[key]) {
            ...
            organiserLink.href = '../../../student/sponsor_pages/sponsor4/sponsor4.html'; // Correct relative path
...
// Function to fetch and display data for event4
function fetchEvent4Data() {
    const event4Ref = ref(database, 'events/event4'); // Reference to 'event4' data in Firebase
    ...
    get(event4Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                displayEventData(eventData); // Display the fetched event4 data
            } else {
                document.getElementById("event4Data").innerHTML = '<p>No data available for event4.</p>';
            }
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
            document.getElementById("event4Data").innerHTML = `<p>Error fetching data: ${error.message}</p>`;
        });
}

// Fetch and display data when the page loads
window.onload = fetchEvent4Data;
