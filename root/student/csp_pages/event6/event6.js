...
function displayEventData(eventData) {
    ...
    const dataDisplayDiv = document.getElementById("event6Data"); // Div to display event6 data
    ...
    displayOrder.forEach(key => {
        ...
        if (key === 'Organiser' && eventData[key]) {
            ...
            organiserLink.href = '../../../student/sponsor_pages/sponsor6/sponsor6.html'; // Correct relative path
...
// Function to fetch and display data for event6
function fetchEvent6Data() {
    const event6Ref = ref(database, 'events/event6'); // Reference to 'event6' data in Firebase
    ...
    get(event6Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                displayEventData(eventData); // Display the fetched event6 data
            } else {
                document.getElementById("event6Data").innerHTML = '<p>No data available for event6.</p>';
            }
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
            document.getElementById("event6Data").innerHTML = `<p>Error fetching data: ${error.message}</p>`;
        });
}

// Fetch and display data when the page loads
window.onload = fetchEvent6Data;
