...
function displayEventData(eventData) {
    ...
    const dataDisplayDiv = document.getElementById("event8Data"); // Div to display event8 data
    ...
    displayOrder.forEach(key => {
        ...
        if (key === 'Organiser' && eventData[key]) {
            ...
            organiserLink.href = '../../../student/sponsor_pages/sponsor8/sponsor8.html'; // Correct relative path
...
// Function to fetch and display data for event8
function fetchEvent8Data() {
    const event8Ref = ref(database, 'events/event8'); // Reference to 'event8' data in Firebase
    ...
    get(event8Ref)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                displayEventData(eventData); // Display the fetched event8 data
            } else {
                document.getElementById("event8Data").innerHTML = '<p>No data available for event8.</p>';
            }
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
            document.getElementById("event8Data").innerHTML = `<p>Error fetching data: ${error.message}</p>`;
        });
}

// Fetch and display data when the page loads
window.onload = fetchEvent8Data;
